...
namespace BestHTTP.WebSocket
{
    public sealed class WebSocketResponse : HTTPResponse, IHeartbeat, IProtocol
    {
        ...
        public void Send(WebSocketFrame frame)
        {
            ...
            if (this.WebSocket.EnableAdditionalLogging)
                HTTPManager.Logger.Verbose("WebSocketResponse", "Queuing frame: " + frame, this.Context);
                        ...
                        try
                        {
                            ...
                            while (this.unsentFrames.TryDequeue(out frame))
                            {
                                if (!closeSent)
                                {
                                    ...
                                    {
                                        if (this.WebSocket.EnableAdditionalLogging)
                                            HTTPManager.Logger.Verbose("WebSocketResponse", "SendThreadFunc - writing data to the buffer. Data length: " + rawData.Length, this.Context);
