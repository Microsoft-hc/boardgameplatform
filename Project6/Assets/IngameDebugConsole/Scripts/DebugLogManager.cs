...
#pragma warning restore 0414
        ...
        [SerializeField]
        private GameObject logShowPanel;
        [SerializeField]
        private Button CloselogShowPanel;
        [SerializeField]
        private Text logShowText;
...
#pragma warning restore 0649
		...
		private void Awake()
		{
			...
			CloselogShowPanel.onClick.AddListener(() => { logShowPanel.SetActive(false); });
		...
		private void OnEnable()
		{
			...
			if (!Show)
			{
                popupManager.transform.position = new Vector3(10000, 10000);
            }
		...
		private void ProcessLog( QueuedDebugLogEntry queuedLogEntry )
		{
			...
			if( !isEntryInCollapsedEntryList )
			{
				...
				{
					logShowPanel.SetActive(true);
					logShowText.text = $"{logEntry.logString}/n {logEntry.stackTrace}"; 
